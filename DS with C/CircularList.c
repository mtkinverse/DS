...
struct Node *InsertAtFirst(struct Node *head, int data)
{
    ...
    while(ptr->nextNode!=head)ptr=ptr->nextNode;
...
struct Node *InsertAtIndex(struct Node *head, int index, int data)
{
    ...
    while (ptr->nextNode != head && i < index-1)
    {
        ptr = ptr->nextNode;
        i++;
    }
...
struct Node* DelteAtIndex(struct Node* head,int index){
    ...
    struct Node* p = head;
    int i=0;
    while(p->nextNode!=head && i<index-1){
        p=p->nextNode;
        i++;
    }
    if(i==index-1){
...
int main()
{
    ...
    DisplayList(FirstNode);
    FirstNode = DeleteAtFirst(FirstNode);
    printf("List after Deleting first index : ");
    DisplayList(FirstNode);
    FirstNode = DelteAtIndex(FirstNode, 6);
    printf("List after deleting from 6th index : ");
    DisplayList(FirstNode);
